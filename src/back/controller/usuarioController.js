...
const loginUsuario = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ mensagem: "Erro interno no servidor ao tentar fazer login." });
  }
};
...
const excluirMinhaConta = async (req, res) => {
  ...
  try {
    ...
    await connection.execute(`DELETE FROM ConsumoRacao WHERE usuario_id = ?`, [
      usuario_id,
    ]);

    // 6. Apagar Rações do usuário
    await connection.execute(`DELETE FROM Racao WHERE usuario_id = ?`, [
      usuario_id,
    ]);

    // 7. Apagar Cachorros do usuário
    await connection.execute(`DELETE FROM Cachorro WHERE usuario_id = ?`, [
      usuario_id,
    ]);
